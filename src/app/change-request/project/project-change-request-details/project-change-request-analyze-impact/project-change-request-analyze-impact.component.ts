...
@Component({
  selector: 'mc-project-change-request-analyze-impact',
  templateUrl: './project-change-request-analyze-impact.component.html',
  styleUrls: ['./project-change-request-analyze-impact.component.scss'],
  animations: [
...
export class ProjectChangeRequestAnalyzeImpactComponent implements OnInit {
  @Input()
  ...
  initSubscribers(): void {
    ...
    this.buttonActionScia$ = this.appStore.pipe(select(selectCaseAction,
      this.storeHelperService.getButtonSelector('ChangeRequest', 'CREATE_SCIA',
        this.changeRequestFormGroup.get('id').value, '')
    ));
  }
